...
const PoemWeaver = () => {
  ...
  const askAI = async (context: string) => {
    // if (!GROQ_API_KEY) return toast.error("Groq API Key is missing.");
    ...
    try {
      ...
      //           role: "system",
      //           content:
      //             "You are a poet collaborating with a human. Continue this poem with exactly ONE line. Keep the same mood and style. Do not add quotes, introductory text, or conversational filler. Just write the next line.",
      //         },
      ...
      //   },
      // );
const res = await api.post('/showcase/poem', { context });
addLine(res.data.text.trim(), 'ai');
...
//       addLine(aiLine.trim(), "ai");
  ...
  const getHint = async () => {
    ...
    try {
      ...
      //           role: "system",
      //           content:
      //             "Give a short, inspiring hint (one sentence max) for the next line of this poem. Be creative.",
      //         },
      //         { role: "user", content: context },
      //       ],
      //       temperature: 0.7,
      ...
      const res = await api.post('/showcase/poem-hint', { context });
setHint(res.data.text.trim());

      // if (!res.ok) throw new Error("Failed to get hint");
      // const data = await res.json();
      // setHint(
      //   data.choices[0]?.message?.content?.trim() ||
      //     "Think about contrast or a surprising word.",
      // );
    } catch {
        ...
        {poemLines.length > 0 && (
          <div className="space-y-2 max-h-96 overflow-y-auto mb-4 p-4 bg-gray-50 rounded-lg border">
            {poemLines.map((line, idx) => (
              <div
                key={idx}
                className={`p-3 rounded shadow-sm ${line.author === "user" ? "bg-green-50 border-l-4 border-green-500" : "bg-orange-50 border-l-4 border-orange-500"}`}
              >
                <span className="font-bold text-xs uppercase tracking-wider text-gray-500 mb-1 block">
                  {line.author === "user" ? "You" : "Groq AI"}
                </span>
                <p className="text-gray-800 whitespace-pre-wrap font-serif text-lg">
                  {line.text}
                </p>
              </div>
            ))}
            {waitingForAI && (
              <div className="p-3 rounded shadow-sm bg-orange-50 border-l-4 border-orange-300 animate-pulse">
                <span className="font-bold text-xs uppercase tracking-wider text-orange-400 mb-1 block">
                  Groq AI
                </span>
