...
const GalleryPage = () => {
  ...
  // Wrapped in useCallback so we can safely run it in useEffect
  // const fetchPhotos = useCallback(async (searchQuery: string) => {
  //   const currentKey = localStorage.getItem('pexels_key');
  //   if (!currentKey) {
  //     setShowKeyInput(true);
  //     return;
  //   }
  ...
  setLoading(true);
  try {
    // Talk to YOUR backend, not Pexels
    const res = await api.get(`/showcase/gallery?query=${encodeURIComponent(searchQuery)}`);
  ...
  // Fetch default photos on mount if key exists
  useEffect(() => {
    if (localStorage.getItem('pexels_key')) {
      fetchPhotos('');
    }
  }, [fetchPhotos]);
    ...
    <PublicLayout title="🖼️ Pexels Gallery">
      ...
      {/* {showKeyInput ? (
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          ...
          <p className="mb-4 text-gray-600 text-sm">
            Enter your free Pexels API Key (from pexels.com/api). It will be saved securely in your browser.
          </p>
          <div className="flex gap-2">
            <input 
              type="password" 
          ...
          {!loading && photos.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {photos.map(photo => (
                <a 
                  key={photo.id} 
                  href={photo.url} // Links directly to Pexels page (per API guidelines)
                  target="_blank" 
                  rel="noreferrer"
                  className="bg-gray-50 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow group relative block"
                >
                  <img 
                    src={photo.src.medium} // Pexels image source path
